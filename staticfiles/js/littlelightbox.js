...
(function(win, doc, $, undefined) {
 ...
 document.addEventListener("DOMContentLoaded", function () {
	...
	const zoomFigure = document.querySelector(".image-zoom");
	const fullscreenImg = document.getElementById("fullscreen-image");

	thumbnailLinks.forEach((thumb) => {
			thumb.addEventListener("click", function () {
					...
					zoomFigure.style.backgroundImage = `url('${newSrc}')`;

					// Update fullscreen image
					fullscreenImg.setAttribute("src", newSrc);
			});
	});
});
        ...
        link.addEventListener('click', function (e) {
            ...
            .then(response => {
                if (response.ok) {
                    ...
                    this.closest('ul.cart-wrap').remove();
                } else {
...
document.addEventListener("DOMContentLoaded", function () {
		// Loop through all quantity sections
		document.querySelectorAll(".cart-plus-minus").forEach(function (quantityBox) {
				const minusBtn = quantityBox.querySelector(".minus");
				const plusBtn = quantityBox.querySelector(".plus");
				const inputField = quantityBox.querySelector("input[name='quantity']");
    ...
    const csrftoken = getCookie('csrftoken');
		...
		$(document).ready(function() {
			$(".remove-from-cart").click(function(e) {
					...
					let itemId = $(this).data("item-id");  // Use `data-item-id` in HTML
					let removeUrl = "/remove-from-cart/" + itemId + "/";
					...
					$.ajax({
							...
							success: function(response) {
									if (response.status === "success") {
											$("#cart-item-" + itemId).remove();  // remove the HTML block
									} else {
